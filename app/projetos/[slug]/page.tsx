  ...
  return (
    <>
      ...
      <div style={{
        ...
        <Link href="/" style={{ color: 'var(--gray-400)', textDecoration: 'none' }}>design.antoniobob.com</Link>
      ...
      {/* HERO */}
      <div style={{
        padding: 'clamp(48px,6vw,96px) clamp(16px,3vw,48px) 0',
        borderBottom: '2px solid var(--black)',
      }}>
        <div style={{
          fontSize: '9px', fontWeight: 700, letterSpacing: '3px',
          textTransform: 'uppercase', color: 'var(--gray-400)', marginBottom: '24px',
        }}>
          Projeto de Pesquisa{projeto.financiamento ? ` · ${projeto.financiamento}` : ''}
        </div>

        <div style={{
          fontSize: 'clamp(48px,10vw,120px)', fontWeight: 900,
        ...
        <div style={{
          ...
          color: 'var(--gray-600)', maxWidth: '560px', lineHeight: 1.7,
          marginBottom: '48px',
        }}>
          {projeto.descricao}
        </div>
          ...
          {[
            ...
            <div key={item.label} style={{
              ...
              <div style={{
                ...
                textTransform: 'uppercase', color: 'var(--gray-400)', marginBottom: '8px',
              }}>{item.label}</div>
              <div style={{ fontSize: '14px', fontWeight: 400 }}>{item.val || '—'}</div>
            </div>
          ))}
        </div>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 280px' }}>
        ...
        <div style={{ borderRight: '1px solid var(--gray-200)' }}>
          <div style={{
            padding: 'clamp(20px,3vw,32px) clamp(16px,3vw,48px)',
            borderBottom: '1px solid var(--gray-200)',
          ...
          {artefatos.length === 0 ? (
            <div style={{
              ...
              fontSize: '13px', fontWeight: 300, color: 'var(--gray-400)',
            }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr' }}>
              {artefatos.map((a: any, i: number) => (
                <Link key={a.slug} href={`/artefatos/${a.slug}`} style={{
                  padding: 'clamp(28px,4vw,40px) clamp(16px,3vw,48px)',
                  ...
                  <div style={{
                    ...
                    background: CORES[a.tipo] || 'var(--gray-400)',
                    marginBottom: '20px',
                  }}></div>
                  <div style={{
                    fontSize: '9px', fontWeight: 700, letterSpacing: '2px',
                    textTransform: 'uppercase', color: 'var(--gray-400)', marginBottom: '8px',
                  }}>{a.tipo.replace(/_/g, ' ')}</div>
                  <div style={{
                    fontSize: 'clamp(14px,2vw,18px)', fontWeight: 700,
                    textTransform: 'uppercase', letterSpacing: '-0.3px', lineHeight: 1.2,
                    marginBottom: '10px',
                  }}>{a.titulo}</div>
                  <div style={{ fontSize: '12px', fontWeight: 300, color: 'var(--gray-400)' }}>
        ...
        {/* SIDEBAR */}
        <div style={{ padding: 'clamp(28px,3vw,40px) clamp(16px,3vw,28px)' }}>
